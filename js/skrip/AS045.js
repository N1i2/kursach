...
str.addEventListener('click', function () {
    if (str.checked) { neob.style.display = "none"; tt.style.display = "block"; tt.innerHTML = "Струн столько же сколько скрипок"; }
...
but.addEventListener('click', function () {
    ...
    if (schet > 1) { text += schet + ' скрипок по 450руб. '; numb += 450 * schet + 'руб.(за гитары) '; first = 450 * schet; }
    else { text += '1 скрипка по 450руб. '; numb += 450 + 'руб.(за гитару) '; first = 450; }

    if (str.checked) { text += '+ ' + schet + ' пачек струн по 55руб. '; numb += '+ ' + 55 * schet + 'руб.(за струны) '; second = 55 * schet; }
    else {
        if (strun > 0) { text += '+ ' + strun + ' пачек струн по 55руб. '; numb += '+ ' + 55 * strun + 'руб.(за струны) '; second = 55 * strun; }
        else { text += '+ без струн '; }
    }
